...
class DictHTTPHandler(tornado.web.RequestHandler):
...
class ImportHandler(DictHTTPHandler):
    ...
    def post(self):
        if self.checkVersion():
            ...
            if bulkExportWasCancelled:
                if self.mw.DictBulkMediaExportWasCancelled and previousBulkTimeStamp == timestamp:
                    self.finish("yes")
                else:
                    self.finish("no")
                return
...
class LearningStatusHandler(DictHTTPHandler):
...
class SearchHandler(DictHTTPHandler):
...
class DictHTTPServer(tornado.web.Application):
    ...
    def checkVersion(self, version):
        if version is False or version < self.PROTOCOL_VERSION:
            self.alert("Your Anki Dictionary Version is newer than and incompatible with your  Browser Extension installation. Please ensure you are using the latest version of the add-on and extension to resolve this issue.")
            return False
        elif version > self.PROTOCOL_VERSION:
            self.alert("Your  Browser Extension Version is newer than and incompatible with this Anki Dictionary installation. Please ensure you are using the latest version of the add-on and extension to resolve this issue.")
            return False
        return True

class DictServerThread(QThread):
    ...
    def __init__(self, mw):
        ...
        self.server = DictHTTPServer(self, mw)
        self.start()
